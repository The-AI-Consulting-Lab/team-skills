...
Usage:
    python3 transcribe_url.py <url> [--model tiny|base|small|medium|large] [--timestamps]

Model sizes (default: tiny):
    tiny    ~75MB  ~5-20s    Best for short clear English speech (cold emails, social clips)
    base    ~150MB ~15-40s   Better accuracy on accents and proper nouns
...
If a transcription comes out garbled (heavy accent, low audio quality, background
noise, technical jargon), re-run with --model base or --model small.

Environment variables:
    TRANSCRIBE_INSECURE_SSL=1   Disable SSL certificate verification in yt-dlp.
                                Set this only when running behind a corporate proxy
                                or in a sandbox that injects self-signed certs into
                                the TLS chain. Default is secure (verify certs).
"""
...
def download_audio(url: str, output_dir: str) -> tuple[str, dict]:
    ...
    # Opt-in escape hatch for environments with self-signed certs in the TLS
    # chain (corporate proxies, sandboxes, dev VMs). Off by default.
    if os.environ.get("TRANSCRIBE_INSECURE_SSL") == "1":
        ydl_opts["nocheckcertificate"] = True
        print(
            "WARN: TRANSCRIBE_INSECURE_SSL=1 set, skipping SSL cert verification",
            file=sys.stderr,
        )
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 transcribe_url.py <url> [--model tiny|base|small|medium|large] [--timestamps]")
        print("")
        print("Default model: tiny (fast, good for short clear English speech)")
        print("For heavy accents, technical jargon, or noisy audio, try --model base or --model small")
        sys.exit(1)
